            ...
            <div class="col-lg-8">
                ...
                <div class="container text-center my-5">
                    @if(isset($members) && count($members) >0)
                        @php($firstId = $members->first()->id)
                        @php($imageData = \App\Http\Controllers\v1\AdminController::getContentImage($members->first()->id))
                        <div class="row justify-content-center my-4">
                            <div class="col-md-4">
                                <div class="member-card">
                                    <img src="{{asset('storage/'.$imageData->document_path)}}" alt="Chairperson">
                                    <div class="member-info">
                                        <div class="member-name">{{$members->first()->full_name}}</div>
                                        <div class="member-role">{{$members->first()->title_p}}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
